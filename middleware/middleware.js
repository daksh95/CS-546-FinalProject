const homeMiddleware = (req, res, next) => {
  ...
  if (req.path === '/login' || req.path.toLowerCase() === '/signup') {
    if (!isAuthenticated) return next();
      ...
      if (isRejected) return res.render('accountRejected', { title: "Account Rejected" });
  ...
  if (req.path.startsWith('/land') || req.path.startsWith('/user') || req.path.startsWith('/transactions')) {
    if (!isAuthenticated) return res.redirect('/login');
    else {
      if (!profileSetUpDone) {
        if (typeOfUser === 'user') {
